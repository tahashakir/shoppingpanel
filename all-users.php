...
$start = ($page - 1) * $limit;
$result = $con->query("SELECT * FROM user  LIMIT $start, $limit");
$customers = $result->fetch_all(MYSQLI_ASSOC);

$result1 = $con->query("SELECT count(userid) AS id FROM user ");
...
<nav aria-label="Page navigation example" class="float-left">
  <ul class="pagination">
    ...
    <li class="page-item"><a class="page-link" href="all-users.php?page=<?= $Previous; ?>" >Previous</a></li>
    <?php for($i = 1; $i<= $pages; $i++) : ?>
    <li class="page-item"><a href="all-users.php?page=<?= $i; ?>" class="page-link"><?= $i; ?></a></li>
    <?php endfor; ?>
    <li class="page-item"><a href="all-users.php?page=<?= $Next; ?>" class="page-link">Next</a></li>
  </ul>
</nav>
...
<td><?=$n?></td>
    ...
    <td>
<a href="myphp.php?delete_users=yes&id=<?=$customer['userid']?>">
...
$(document).ready(function(){
    $('.search-box input[type="text"]').on("keyup input", function(){
        ...
        if(inputVal.length){
            $.get("search-users.php", {term: inputVal}).done(function(data){
